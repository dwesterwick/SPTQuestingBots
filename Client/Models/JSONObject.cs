        ...
        public void UpdateJSONProperty(string jsonPropertyName, object val, BindingFlags bindingFlags)
        {
            ...
            if (val.GetType() == typeof(JArray))
            {
                try
                {
                    ...
                    object? valTmp = array.ToObject(matchingProperties[0].PropertyType);
                    if (valTmp == null)
                    {
                        throw new InvalidOperationException("Could not convert object to type " + matchingProperties[0].PropertyType.FullName);
                    }
            ...
            if (val.GetType() == typeof(JObject))
            {
                try
                {
                    ...
                    object? valTmp = jObject.ToObject(matchingProperties[0].PropertyType);
                    if (valTmp == null)
                    {
                        throw new InvalidOperationException("Could not convert object to type " + matchingProperties[0].PropertyType.FullName);
                    }
