...
namespace QuestingBots.Utils.Benchmarking
{
    internal static class BenchmarkService
    {
        ...
        public static long GetCurrentlyAllocatedMemory() => GC.GetTotalMemory(false);
        ...
        public static void LogAllBenchmarksAndReset(long timestamp)
        {
            ...
            if (QuestingBotsPluginConfig.EnableBenchmarking.Value && (_results.Count > 0))
            {
                ...
                //Singleton<LoggingUtil>.Instance.LogDebug(logText);
        ...
        private static string CreateBenchmarkingLogText()
        {
            ...
            sb.AppendLine("Overall Time when Completed (ms),Method Name,ET (ms), Allocation (bytes)");

            foreach (BenchmarkResult result in _results)
            {
                sb.AppendLine(result.StartTime + "," + result.GetMethodName() + "," + result.ElapsedMilliseconds + "," + result.AllocatedMemory);
            }
