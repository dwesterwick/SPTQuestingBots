...
namespace SPTQuestingBots.Patches
{
    public class IsFollowerSuitableForBossPatch : ModulePatch
    {
        ...
        private static bool PatchPrefix(ref bool __result, BotBoss __instance, BotOwner offer)
        {
            ...
            if (Singleton<Controllers.Bots.Spawning.PMCGenerator>.Instance.SpawnedGroupCount > 0)
            {
                if (!Singleton<Controllers.Bots.Spawning.PMCGenerator>.Instance.TryGetBotGroup(offer, out Models.BotSpawnInfo botSpawnInfo))
                {
                    return true;
                }

                groupMembers = Singleton<Controllers.Bots.Spawning.PMCGenerator>.Instance.GetSpawnGroupMembers(__instance.Owner);
            }
            else
            {
