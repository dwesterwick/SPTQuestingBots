...
namespace SPTQuestingBots.Patches
{
    public class IsFollowerSuitableForBossPatch : ModulePatch
    {
        ...
        private static bool PatchPrefix(ref bool __result, BotBoss __instance, BotOwner offer)
        {
            ...
            if ((pmcGenerator != null) && (pmcGenerator.SpawnedGroupCount > 0))
            {
                if (!pmcGenerator.TryGetBotGroup(offer, out Models.BotSpawnInfo botSpawnInfo))
                {
                    return true;
                }

                groupMembers = pmcGenerator.GetSpawnGroupMembers(__instance.Owner);
            }
            else
            {
