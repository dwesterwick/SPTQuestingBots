...
* They're trying to extract (using [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement))
...
These three metrics are then added together, and the result is the overall score for the corresponding quest. The quest with the highest score is assigned to the bot. If for some reason the bot is unable to perform that quest, it selects the one with the next-highest score, and so on. If no quests are available for the bot to select, this mod will first try allowing the bot to perform repeatable quests early (before the **questing.bot_questing_requirements.repeat_quest_delay** delay expires). If there are no available repeatable quests, this mod will then attempt to make the bot extract via [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement). Finally, this mod will stop assigning new quests to the bot. 
...
* **Objectives**: An objective is a collection of at least one step. An objective represents a list of actions that the bot must complete in the order you specify. 
    ...
    * **lootAfterCompleting**: The only valid options for this are "Default", "Force", and "Inhibit" (case-sensitive). If "Force" is used, Questing Bots will try invoking [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots) to make the bot scan for loot immediately after completing each step in the objective. However, bots will not be able to loot if they're in combat or have no available space. If "Inhibit" is used, this mod will try invoking [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots) to prevent the bot from looting until after it selects another quest objective. [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots) version 1.2.1 or later is required for either option to work. [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement) 2.1.9 or later is required for Questing Bots to properly force bots to loot.
...
* **Steps**: A step is an individual component of an objective. 
    ...
    * **stepType**: The only valid options for this are (case-sensitive):
        ...
        * **RequestExtract**: This mod will try to instruct bots to extract via [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement).
...
* **questing.bot_questing_requirements.break_for_looting.min_time_between_looting_events**: The minimum delay (in seconds) after a bot successfully finds loot before it will be allowed to take a break again. If this value is very low, bots may frequently back-track and may never reach their objectives. If this value is high, bots will rarely loot. This supersedes **bot_questing_requirements.break_for_looting.min_time_between_looting_checks**, and it requires [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots) (or it will be ignored). This is **80** s by default. 
* **questing.bot_questing_requirements.break_for_looting.max_time_to_start_looting**: The duration of each break (in seconds). If one of the [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots) brain layers is not active after this time, the bot will resume questing. This is **2** s by default. 
* **questing.bot_questing_requirements.break_for_looting.max_loot_scan_time**: The maximum time that bots will be allowed to search for loot via [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots). If the bot hasn't found any loot within this time, it will continue questing. If it has found loot, it will not continue questing until it's completely finished with looting. This is **4** s by default. 
...
* **questing.extraction_requirements.use_sain_for_extracting**: If [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement) should be used to control extract behavior for bots. Otherwise, vanilla EFT logic will be used. This is **false** by default. 
...
* Thanks to [Props](https://forge.sp-tarkov.com/user/15516/props) for sharing the code [DONUTS](https://forge.sp-tarkov.com/mod/644/swag-donuts-dynamic-spawn-waves-and-custom-spawn-points) uses to spawn bots. This was the inspiration to create this mod. 
* Thanks to [DrakiaXYZ](https://forge.sp-tarkov.com/user/27605/drakiaxyz) for creating [BigBrain](https://forge.sp-tarkov.com/mod/902/bigbrain) and [Waypoints](https://forge.sp-tarkov.com/mod/827/waypoints-expanded-navmesh) and for all of your help with developing this mod. Also, thanks for your help with adding interop capability to [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement).
* Thanks to [nooky](https://forge.sp-tarkov.com/user/25830/nooky) for lots of help with testing and ensuring this mod remains compatible with [SWAG + DONUTS](https://forge.sp-tarkov.com/mod/644/swag-donuts-dynamic-spawn-waves-and-custom-spawn-points). 
* Thanks to [Skwizzy](https://forge.sp-tarkov.com/user/28069/skwizzy) for help with adding interop capability to [Looting Bots](https://forge.sp-tarkov.com/mod/812/looting-bots).
* Thanks to [Solarint](https://forge.sp-tarkov.com/user/27463/solarint) for help with improving interop capability to [SAIN](https://forge.sp-tarkov.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement) and working with me to balance bot questing vs. combat behavior.
* Thanks to [ozen](https://forge.sp-tarkov.com/user/77756/ozen) for help with testing and contributing to SPT 3.11 and SPT 4.0 updates.
