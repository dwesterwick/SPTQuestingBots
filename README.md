...
* **questing.stuck_bot_detection.min_time_before_jumping**: The minimum time (in seconds) the bot is allowed to move less than **questing.stuck_bot_detection.distance** meters while questing before it will try jumping to prevent itself from being stuck. This is **5** s by default.
* **questing.stuck_bot_detection.jump_debounce_time**: The minimum time (in seconds) between a bot's jump attempts to prevent itself from being stuck. This is **3** s by default. 
