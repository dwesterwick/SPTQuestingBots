...
* [Looting Bots](https://sp-mod.com/mod/812/looting-bots) (1.8.0 or later recommended)

**Partially compatible with:**
* [Vagabond](https://sp-mod.com/mod/2642/vagabond) and similar mods that significantly reduce spawn points (You MUST use another mod like [Acid's Bot Placement System](https://sp-mod.com/mod/2097/abps-acids-bot-placement-system) to manage bot spawning when using this mod. Otherwise, bots will spawn right in front of you.) 

**NOT compatible with:**
* [Phobos](https://github.com/SleepingPills/Phobos)
* [ORBIT](https://sp-mod.com/mod/2706/orbit)
* [AI Limit](https://sp-mod.com/mod/1945/ai-limit) or any other mods that disable AI in a similar manner. This mod relies on the AI being active throughout the entire map. **Starting with 0.2.10, Questing Bots has its own AI Limiter feature.** Please see the tab below for more information.
* [QuestManiac](https://sp-mod.com/mod/535/andrudis-questmaniac)
...
* [Unda](https://sp-mod.com/mod/1173/unda)
* [Acid's Bot Placement System](https://sp-mod.com/mod/2097/abps-acids-bot-placement-system)
...
* They're trying to extract (using [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement))
...
These three metrics are then added together, and the result is the overall score for the corresponding quest. The quest with the highest score is assigned to the bot. If for some reason the bot is unable to perform that quest, it selects the one with the next-highest score, and so on. If no quests are available for the bot to select, this mod will first try allowing the bot to perform repeatable quests early (before the **questing.bot_questing_requirements.repeat_quest_delay** delay expires). If there are no available repeatable quests, this mod will then attempt to make the bot extract via [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement). Finally, this mod will stop assigning new quests to the bot. 
...
* **Objectives**: An objective is a collection of at least one step. An objective represents a list of actions that the bot must complete in the order you specify. 
    ...
    * **lootAfterCompleting**: The only valid options for this are "Default", "Force", and "Inhibit" (case-sensitive). If "Force" is used, Questing Bots will try invoking [Looting Bots](https://sp-mod.com/mod/812/looting-bots) to make the bot scan for loot immediately after completing each step in the objective. However, bots will not be able to loot if they're in combat or have no available space. If "Inhibit" is used, this mod will try invoking [Looting Bots](https://sp-mod.com/mod/812/looting-bots) to prevent the bot from looting until after it selects another quest objective. [Looting Bots](https://sp-mod.com/mod/812/looting-bots) version 1.2.1 or later is required for either option to work. [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement) 2.1.9 or later is required for Questing Bots to properly force bots to loot.
...
* **Steps**: A step is an individual component of an objective. 
    ...
    * **stepType**: The only valid options for this are (case-sensitive):
        ...
        * **RequestExtract**: This mod will try to instruct bots to extract via [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement).
...
* **questing.quest_selection.max_calc_time_per_frame_ms**: The maximum amount of time (in milliseconds) the mod is allowed to run quest-selection procedures per frame. By default this is set to **1** ms.
* **questing.quest_selection.timeout**: If a quest cannot be selected for a bot after trying for this amount of time (in seconds), the mod will give up and write an error message.
...
* **questing.stuck_bot_detection.stuck_bot_remedies.min_time_before_nav_mesh_snapping**: The minimum time (in seconds) after a bot is allowed to move less than **questing.stuck_bot_detection.distance** meters while questing before it can allow EFT to snap its position to the NavMesh. This is **4** s by default.
...
* **questing.bot_zone_updates.update_bot_zone_after_stopping**: Whenever a bot stops going to its quest objective (for any reason), check which BotZone is closest to the bot's current location and assign it to the bot's group (if it's different). This prevents bots from running away from their current location whenever they take a break from questing. This check will only be performed for solo bots or bosses of bot groups. Otherwise, followers may cause the whole group to retreat if they get stuck somewhere. This is **true** by default. 
* **questing.bot_zone_updates.update_patrol_point_after_stopping**: Whenever a bot stops going to its quest objective (for any reason), update its target patrol point to be as close as possible. This makes them less likely to wander away from their quest positions. This is **true** by default. 
* **questing.bot_zone_updates.debounce_time_after_changing_patrol_point_s**: After a bot updates its patrol point when **questing.bot_zone_updates.update_patrol_point_after_stopping=true**, it must wait this amount of time before updating it again. This is **0.5** s by default.
* **questing.bot_zone_updates.patrol_point_radius_around_boss.min**: When selecting a new patrol point if **questing.bot_zone_updates.update_patrol_point_after_stopping=true**, bots will not be allowed to choose one within this radius of their boss (if applicable). This is **3** m by default.
* **questing.bot_zone_updates.patrol_point_radius_around_boss.max**: When selecting a new patrol point if **questing.bot_zone_updates.update_patrol_point_after_stopping=true**, bots will not be allowed to choose one that is more than this distance away from them. This is **50** m by default. 
...
* **questing.bot_pathing.eft_local_avoidance.allow_for_questing_bots**: If EFT's "local avoidance" system should be enabled while bots are questing. When enabled, this system prevents bots from being too close to each other by adding "inertia" to oppose them. While questing, Questing Bots reduces the strength and sensitivity of this system to allow bots to move more easily. This is **true** by default.
* **questing.bot_pathing.eft_local_avoidance.avoidance_radius.min/max**: Overrides for EFT's local-avoidance thresholds defining how close bots must be to each other before "inertia" is applied to oppose them. **Do not change this unless you know what you're doing!**
* **questing.bot_pathing.eft_local_avoidance.repulsion_power_limit**: Override for EFT's local-avoidance threshold defining how much bot should oppose each other when they are within **questing.bot_pathing.eft_local_avoidance.avoidance_radius.min/max** of each other. This is **0.05** by default. **Do not change this unless you know what you're doing!**
* **questing.bot_pathing.eft_local_avoidance.radius_multiplier_to_drop_offset**: If a bot has no other bots within the radius of **questing.bot_pathing.eft_local_avoidance.avoidance_radius.max** multiplied by this value, all local-avoidance "inertia" will be reset for it. This is **2** by default. **Do not change this unless you know what you're doing!**
...
* **questing.bot_questing_requirements.break_for_looting.min_time_between_looting_events**: The minimum delay (in seconds) after a bot successfully finds loot before it will be allowed to take a break again. If this value is very low, bots may frequently back-track and may never reach their objectives. If this value is high, bots will rarely loot. This supersedes **bot_questing_requirements.break_for_looting.min_time_between_looting_checks**, and it requires [Looting Bots](https://sp-mod.com/mod/812/looting-bots) (or it will be ignored). This is **80** s by default. 
* **questing.bot_questing_requirements.break_for_looting.max_time_to_start_looting**: The duration of each break (in seconds). If one of the [Looting Bots](https://sp-mod.com/mod/812/looting-bots) brain layers is not active after this time, the bot will resume questing. This is **2** s by default. 
* **questing.bot_questing_requirements.break_for_looting.max_loot_scan_time**: The maximum time that bots will be allowed to search for loot via [Looting Bots](https://sp-mod.com/mod/812/looting-bots). If the bot hasn't found any loot within this time, it will continue questing. If it has found loot, it will not continue questing until it's completely finished with looting. This is **4** s by default. 
...
* **questing.extraction_requirements.use_sain_for_extracting**: If [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement) should be used to control extract behavior for bots. Otherwise, vanilla EFT logic will be used. This is **false** by default. 
...
* **bot_spawns.player_scav_brain_conversion_chances_overrides.enabled**: Allows this mod to override the chances that player Scav brains will be replaced by another brain type. This is **true** by default.
...
* Bots sometimes teleport through doors, which also sometimes "breaks" them. 
...
* Looting via Questing Bots only allows bots to travel toward loot POI's (via vanilla EFT logic), so they rarely loot while questing. Using [Looting Bots](https://sp-mod.com/mod/812/looting-bots) will result in more realistic behavior. 

**---------- Credits ----------**

* Thanks to [Props](https://sp-mod.com/user/15516/props) for sharing the code [DONUTS](https://sp-mod.com/mod/644/swag-donuts-dynamic-spawn-waves-and-custom-spawn-points) uses to spawn bots. This was the inspiration to create this mod. 
* Thanks to [DrakiaXYZ](https://sp-mod.com/user/27605/drakiaxyz) for creating [BigBrain](https://sp-mod.com/mod/902/bigbrain) and [Waypoints](https://sp-mod.com/mod/827/waypoints-expanded-navmesh) and for all of your help with developing this mod. Also, thanks for your help with adding interop capability to [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement).
* Thanks to [nooky](https://sp-mod.com/user/25830/nooky) for lots of help with testing and ensuring this mod remains compatible with [SWAG + DONUTS](https://sp-mod.com/mod/644/swag-donuts-dynamic-spawn-waves-and-custom-spawn-points). 
* Thanks to [Skwizzy](https://sp-mod.com/user/28069/skwizzy) for help with adding interop capability to [Looting Bots](https://sp-mod.com/mod/812/looting-bots).
* Thanks to [Solarint](https://sp-mod.com/user/27463/solarint) for help with improving interop capability to [SAIN](https://sp-mod.com/mod/791/sain-solarints-ai-modifications-full-ai-combat-system-replacement) and working with me to balance bot questing vs. combat behavior.
* Thanks to [ozen](https://sp-mod.com/user/77756/ozen) for help with testing and contributing to SPT 3.11 and SPT 4.0 updates.
