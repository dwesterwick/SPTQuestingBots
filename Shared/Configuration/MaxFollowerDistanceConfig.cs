...
namespace QuestingBots.Configuration
{
    ...
    {
        [DataMember(Name = "max_wait_time", IsRequired = true)]
        public float MaxWaitTime { get; set; } = 5;

        [DataMember(Name = "min_regroup_time", IsRequired = true)]
        public float MinRegroupTime { get; set; } = 1;

        [DataMember(Name = "regroup_pause_time", IsRequired = true)]
        public float RegroupPauseTime { get; set; } = 2;

        [DataMember(Name = "target_position_variation_allowed", IsRequired = true)]
        public float TargetPositionVariationAllowed { get; set; } = 1;

        [DataMember(Name = "target_range_questing", IsRequired = true)]
        public MinMaxConfig TargetRangeQuesting { get; set; } = new MinMaxConfig();

        [DataMember(Name = "target_range_combat", IsRequired = true)]
        public MinMaxConfig TargetRangeCombat { get; set; } = new MinMaxConfig();

        [DataMember(Name = "nearest", IsRequired = true)]
        public float Nearest { get; set; } = 35;

        [DataMember(Name = "furthest", IsRequired = true)]
